...
parser.add_argument('--img_path', type=str, default="/data/kepler/FiveK/")
...
parser.add_argument('--snapshots_folder', type=str, default="/output/snapshots_folder_lol")
...
wandb.init(project="paper1_Experiences")
wandb.config = {
...
# Data Setting
train_dataset = mit5k_loader(root=config.img_path, mode='train')
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=8,
                                           pin_memory=True)
val_dataset = mit5k_loader(root=config.img_path, mode='test')
...
#         low_img, high_img, name = imgs['im_input'].cuda(), imgs['im_expert'].cuda(), imgs['im_name']
