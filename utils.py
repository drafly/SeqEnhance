...
def get_mean_ssim_and_psnr(im_batch1, im_batch2):
    """
    传入batch则返回均值
    """
    ssim_list, psnr_list = [], []
    
    for i in range(im_batch1.shape[0]):
        im1, im2 = im_batch1[i, :], im_batch2[i, :]
        ssim_value = ssim(im1.unsqueeze(0), im2.unsqueeze(0), as_loss=False).item()
        psnr_value = psnr(im1, im2).item()
        ssim_list.append(ssim_value), psnr_list.append(psnr_value)
...
def validation(model, val_loader):
    ...
    for i, imgs in enumerate(val_loader):
        with torch.no_grad():
            im_low, im_high = imgs[0].cuda(), imgs[1].cuda()
            
            im_enhanced = model(im_low)
        
        ssim_value, psnr_value = get_mean_ssim_and_psnr(im_enhanced, im_high)
        ssim_list.append(ssim_value), psnr_list.append(psnr_value)
